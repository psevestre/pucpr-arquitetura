...
type RestOf<T extends IComment | NewComment> = Omit<T, 'creationDate' | 'publishDate'> & {
...
export class CommentService {
  ...
  protected convertDateFromClient<T extends IComment | NewComment | PartialUpdateComment>(comment: T): RestOf<T> {
    return {
      ...
      publishDate: comment.publishDate?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restComment: RestComment): IComment {
    return {
      ...
      publishDate: restComment.publishDate ? dayjs(restComment.publishDate) : undefined,
    };
  }
