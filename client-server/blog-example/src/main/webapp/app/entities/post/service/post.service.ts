...
type RestOf<T extends IPost | NewPost> = Omit<T, 'creationDate' | 'publishDate'> & {
...
export class PostService {
  ...
  protected convertDateFromClient<T extends IPost | NewPost | PartialUpdatePost>(post: T): RestOf<T> {
    return {
      ...post,
      creationDate: post.creationDate?.toJSON() ?? null,
      publishDate: post.publishDate?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restPost: RestPost): IPost {
    return {
      ...restPost,
      creationDate: restPost.creationDate ? dayjs(restPost.creationDate) : undefined,
      publishDate: restPost.publishDate ? dayjs(restPost.publishDate) : undefined,
    };
  }
